#!/usr/bin/env bash
# 在容器内运行,验证 CARLA 0.9.16 镜像的环境完整性
set -euo pipefail

echo "═══════════════════════════════════════════════════════════════"
echo "  LEAD Docker Smoke Test (CARLA 0.9.16)"
echo "═══════════════════════════════════════════════════════════════"
...
echo ""
echo "[3/4] CARLA 0.9.16 + scenario_runner"
check "CARLA installed"    "test -f /opt/carla/CarlaUE4.sh"
check "CARLA Python API"   "python -c 'import carla; from importlib.metadata import version; print(version(\"carla\"))' 2>&1 | grep -qE '0\.9\.16'"
check "CARLA_ROOT set"     "test -n \"\$CARLA_ROOT\""
check "PYTHONPATH OK"      "echo \$PYTHONPATH | grep -q carla"
check "scenario_runner v0.9.16" "git -C /opt/scenario_runner describe --tags 2>/dev/null | grep -q 'v0.9.16'"
check "SCENARIO_RUNNER_ROOT set" "test -n \"\$SCENARIO_RUNNER_ROOT\""
check "srunner importable" "python -c 'import srunner'"

echo ""
echo "[4/4] LEAD workspace"
check "workspace mounted"  "test -d /workspace/lead"
check "CARLA symlink"      "test -L /workspace/lead/3rd_party/CARLA_0916 || test -d /workspace/lead/3rd_party/CARLA_0916"
